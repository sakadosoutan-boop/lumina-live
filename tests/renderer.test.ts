...
function rig(assets = builtInAssets, options = {}) {
  ...
  return {
    canvas,
    renderer,
    current: state(assets[0].id),
    gl: canvas.context!,
  };
}

describe("renderer cadence and lifecycle", () => {
  it("retires a missing-asset diagnostic only when that asset arrives", () => {
    const r = rig();
    r.current.decks[0].assetId = "loading-a";
    r.current.decks.push(deck("missing-b"));
    r.current.crossfade = 0.5;
    r.renderer.render(r.current);
    expect(r.renderer.getStats().mediaErrors).toEqual([
      "Missing asset: loading-a",
      "Missing asset: missing-b",
    ]);
    r.renderer.setAssets([
      ...builtInAssets,
      { ...builtInAssets[0], id: "loading-a" },
    ]);
    r.renderer.render(r.current);
    expect(r.renderer.getStats().mediaErrors).toEqual([
      "Missing asset: missing-b",
  ...
  it("measures exact 30/60 FPS intervals, counts real misses, and resets samples when the target changes", () => {
    ...
    expect(r.renderer.getStats()).toMatchObject({
      fps: 30,
      dropped: 0,
      mediaErrors: [],
    });
    ...
    for (const invalid of [0, -1, NaN, Infinity])
      r.renderer.setTargetFps(invalid);
    now += 1000 / 60;
    r.renderer.render(r.current);
    expect(r.renderer.getStats().dropped).toBe(3);
    r.renderer.dispose();
  ...
  it("excludes pause/freeze/blackout/hidden transitions and suspension from late counts", () => {
    ...
    for (const mode of ["freeze", "blackout", "pause", "hidden"]) {
      r.current.freeze = mode === "freeze";
      r.current.blackout = mode === "blackout";
      r.current.playing = mode !== "pause";
      document.hidden = mode === "hidden";
      now += 200;
      r.renderer.render(r.current);
      now += 200;
      r.renderer.render(r.current);
      r.current.freeze = r.current.blackout = false;
...
describe("video clock and resource handling", () => {
  ...
  it("reports off-air video readiness only after assignment, decoding and seeking complete", () => {
    ...
    expect(r.renderer.isDeckReady(1)).toBe(false);
    video.seeking = false;
    expect(r.renderer.isDeckReady(1)).toBe(true);
    expect(video.play).not.toHaveBeenCalled(); // B is preloaded, not playing.
    for (const slot of [-1, 2, 3, 0.5, NaN])
      expect(r.renderer.isDeckReady(slot)).toBe(false);
    video.onerror?.();
    expect(r.renderer.isDeckReady(1)).toBe(false);
    r.renderer.setAssets([
      builtInAssets[0],
      { ...videoAsset, url: "blob:replacement" },
    ]);
    expect(r.renderer.isDeckReady(1)).toBe(false);
    r.renderer.render(r.current);
    expect(r.renderer.isDeckReady(1)).toBe(true);
  ...
  it("rejects incomplete/broken images, removed assignments, and zero-opacity media", () => {
    ...
    r.current.decks = [deck(builtInAssets[0].id), deck(imageAsset.id)];
    r.renderer.render(r.current);
    const image = document.images[0];
    expect(r.renderer.isDeckReady(1)).toBe(false);
    image.complete = true;
    expect(r.renderer.isDeckReady(1)).toBe(false);
    image.naturalWidth = image.naturalHeight = 64;
    expect(r.renderer.isDeckReady(1)).toBe(true);
    r.current.decks[1].opacity = 0;
    r.renderer.render(r.current);
    expect(r.renderer.isDeckReady(1)).toBe(false);
    r.current.decks = [];
    r.renderer.render(r.current);
  ...
  it("fits video duration to beats, wraps seeks, and uploads only decoded/dirty frames", async () => {
    ...
    now += 100;
    r.renderer.render(r.current);
    expect(r.gl.api.texImage2D).toHaveBeenCalledTimes(uploads);
    expect(r.gl.api.texSubImage2D).not.toHaveBeenCalled();
    video.frame?.();
    now += 100;
    r.renderer.render(r.current);
  ...
  it("pauses late play promises, releases changed clips during blackout, and avoids starting hidden decoders", async () => {
    ...
    r.current.blackout = true;
    r.renderer.render(r.current);
    expect(document.videos).toHaveLength(0);
    r.current.blackout = false;
    r.current.playing = false;
    r.renderer.render(r.current);
    const video = document.videos[0];
...
describe("chunked procedural thumbnails using the real renderer with mocked GL", () => {
  it("renders all 96 original presets at 240×135, yields bounded batches and releases one context", async () => {
    vi.useFakeTimers();
    const before = structuredClone(builtInAssets),
      batches: number[] = [],
      ticks: number[] = [];
    const job = generateProceduralThumbnails(undefined, {
      onBatch: (batch) => {
    ...
    expect(canvas.snapshots).toEqual(
      Array.from({ length: 96 }, () => [240, 135]),
    );
    const gl = document.contexts[0];
    const proceduralDraws = gl.draws.filter((draw) =>
      draw.uniforms.has("uSeed"),
    );
    expect(proceduralDraws).toHaveLength(96);
    expect(
      proceduralDraws.map((draw) => draw.uniforms.get("uSeed")![0]),
    ).toEqual(builtInAssets.map((asset) => asset.seed));
    expect(
      new Set(proceduralDraws.map((draw) => draw.uniforms.get("uVariant")![0]))
        .size,
    ).toBe(8);
    expect(
      new Set(proceduralDraws.map((draw) => draw.uniforms.get("uHue")![0]))
        .size,
    ).toBe(8);
    expect(
      new Set(proceduralDraws.map((draw) => draw.uniforms.get("uTime")![0]))
        .size,
    ).toBe(8);
    expect(gl.remaining()).toBe(0);
    expect(gl.extension.loseContext).toHaveBeenCalledTimes(1);
  ...
  it("skips nonprocedural/duplicate IDs and does no work when already aborted", async () => {
    ...
    controller.abort();
    await expect(
      generateProceduralThumbnails(undefined, { signal: controller.signal }),
    ).rejects.toMatchObject({ name: "AbortError" });
    expect(document.contexts).toHaveLength(0);
    expect(
      await generateProceduralThumbnails([
        { ...builtInAssets[0], kind: "video" },
      ]),
    ).toEqual(new Map());
    const job = generateProceduralThumbnails([
      builtInAssets[0],
      builtInAssets[0],
    ]);
    await vi.runAllTimersAsync();
    expect((await job).size).toBe(1);
  });

  it.each(["abort", "callback", "webgl"] as const)(
    ...
    async (reason) => {
      ...
      for (const context of document.contexts) {
        ...
        expect(context.extension.loseContext).toHaveBeenCalledTimes(1);
      }
