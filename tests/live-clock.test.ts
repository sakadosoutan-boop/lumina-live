...
describe("live and audio synchronization", () => {
  it("accelerates a rehearsed timeline after a live tempo change without jumping", () => {
    ...
    t.play(0);
    const pos = t.getTime(10000);
    const old = { time: 0, beat: 0, bpm: 120, rate: 1 };
    const rate = timelineRate({ bpm: 150, timelineBpm: 120 }, "live");
    const anchor = { time: pos, beat: beatAt(pos, old), bpm: 150, rate };
    t.setRate(rate, 10000);
    expect(t.getTime(10000)).toBe(10);
    expect(t.getTime(14000)).toBe(15);
  ...
  it("quantizes to the next beat after a nonzero tempo anchor", () => {
    const a = { time: 10, beat: 21, bpm: 90, rate: 0.75 };
  ...
  it("opens the same standalone file with an output query", () => {
    expect(outputUrl("file:///C:/Live%20show/Lumina.html")).toBe(
      "file:///C:/Live%20show/Lumina.html?output=1",
    );
    expect(outputUrl("http://127.0.0.1:4173/?a=2#cue")).toBe(
      "http://127.0.0.1:4173/?a=2&output=1",
    );
  });
});
