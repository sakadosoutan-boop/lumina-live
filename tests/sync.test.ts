...
import {
  activeCue, calculatePlaybackRate, distributeLyrics, exportLrc, exportSrt,
  formatTime, parseDuration, parseLrc, parseSrt, quantizeTime, similarAssets,
  TapTempo, Transport,
} from '../src/sync';

const cue = (start: number, end: number, text = 'lyric', id = `${start}-${end}-${text}`): LyricCue =>
  ({ id, start, end, text });
const timings = (cues: LyricCue[]) => cues.map(({ start, end, text }) => ({ start, end, text }));
const asset = (id: string, values: Partial<Asset> = {}): Asset => ({
  id, name: id, kind: 'procedural', tags: [], hue: 0, energy: 0.5, license: 'CC0', ...values,
});

describe('duration parsing and editable time formatting', () => {
  it.each([
    ['0', 0], [' 125.25 ', 125.25], ['.5', 0.5], ['2:05.125', 125.125],
    ['1:02:03.5', 3723.5], ['120:00', 7200], ['0:00', 0],
  ])('parses %s as seconds', (text, seconds) => {
    expect(parseDuration(text)).toBe(seconds);
  });

  it.each(['', ' ', '-2', 'NaN', 'Infinity', '1e3', '1:60', '1:60:00', '1::20', '1:2:3:4', '2:00junk', '0x20'])('rejects %j', text => {
  ...
  it.each([-1, NaN, Infinity, -Infinity])('formats invalid time %s safely', seconds => {
...
describe('rough lyric distribution', () => {
  it('fills the duration with deterministic, contiguous estimated cues', () => {
    ...
    expect(cues.every(item => item.section?.includes('estimated'))).toBe(true);
  ...
  it('uses BPM/meter for the bar grid and gives longer lines more time', () => {
    expect(distributeLyrics('one\ntwo\nthree', 10, 60).map(item => item.start)).toEqual([0, 4, 8]);
    expect(distributeLyrics('one\ntwo\nthree', 10, 120, 3).map(item => item.start)).toEqual([0, 3, 6]);
  ...
  it('scales overlong instrumental requests and avoids zero-length cues on short songs', () => {
    ...
    expect(distributeLyrics('[instrumental:8]', 10, 120)).toMatchObject([{ start: 0, end: 10, text: '' }]);
  });

  it('handles leading/trailing and adjacent instrumental markers', () => {
    for (const text of ['[instrumental:2]\na\nb', 'a\n[instrumental:2]', 'a\n[instrumental:2]\n[instrumental:2]\nb']) {
      const cues = distributeLyrics(text, 25, 120);
      expect(cues.at(-1)?.end).toBe(25);
      for (const item of cues.filter(item => !item.text)) expect(item.end - item.start).toBeCloseTo(4);
    }
  });
...
describe('LRC import and export', () => {
  ...
  it('applies the last valid global offset even after the lyrics', () => {
    expect(timings(parseLrc('[offset:100]\n[00:01.250]first\n[00:02.250]second\n[offset:+500]\n[offset:bad]'))).toEqual([
  ...
  it('clamps negative offsets before deduplication and uses the next distinct boundary', () => {
    expect(timings(parseLrc('[00:00.100]same\n[00:00.200]same\n[00:03]next\n[offset:-500]'))).toEqual([
  ...
  it('exports sorted millisecond timestamps and round-trips finite ends and gaps', () => {
    ...
    expect(text).toBe('[00:01.001]first\n[00:03.000]\n[01:05.125]second\n[01:06.250]');
  ...
  it('avoids clearing contiguous/overlapping lyrics early, and flattens multiline text', () => {
    expect(exportLrc([cue(0, 2, 'one\ntwo'), cue(2, 4, 'three')])).toBe('[00:00.000]one two\n[00:02.000]three\n[00:04.000]');
  ...
  it('skips invalid/blank/unrepresentably short cues and carries rounded milliseconds', () => {
    expect(exportLrc([cue(-1, 2), cue(2, 2), cue(3, Infinity), cue(NaN, 4), cue(1, 2, ''), cue(0, 0.0001)])).toBe('');
    expect(exportLrc([cue(59.9999, 61, 'carry')])).toBe('[01:00.000]carry\n[01:01.000]');
...
describe('SRT import and export', () => {
  ...
  it('preserves overlaps and distinct duplicate timestamps but removes exact repeated blocks', () => {
    ...
    expect(timings(parseSrt(input))).toEqual([
      { start: 1, end: 4, text: 'a' }, { start: 1, end: 3, text: 'b' },
    ]);
  });
  ...
  it('skips malformed, reversed, zero-duration and empty blocks without losing valid blocks', () => {
    ...
    expect(timings(parseSrt(input))).toEqual([{ start: 3, end: 4, text: 'good' }]);
    expect(parseSrt('')).toEqual([]);
  });

  it('round-trips exact overlapping intervals/multiline text and does not wrap hours', () => {
    const cues = [cue(360000, 360001.125, 'late'), cue(1, 4, 'line\ntwo'), cue(2, 3, 'overlap')];
    const before = structuredClone(cues);
    const output = exportSrt(cues);
    expect(output).toContain('100:00:00,000 --> 100:00:01,125');
    expect(output).toMatch(/^1\n00:00:01,000 --> 00:00:04,000/);
    expect(timings(parseSrt(output))).toEqual(timings([cues[1], cues[2], cues[0]]));
    expect(cues).toEqual(before);
...
describe('active lyric intervals', () => {
  it('handles editable unsorted cues, gaps, and exact half-open boundaries', () => {
    ...
    expect(activeCue(cues, 2)).toEqual({ cue: cues[1], index: 1, progress: 0.5 });
  ...
  it('resolves overlaps by latest start then last source index and restores underlying intervals', () => {
    const cues = [cue(0, 10, 'base'), cue(2, 6, 'new'), cue(2, 4, 'translation')];
  ...
  it('ignores invalid times/intervals and supports blank instrumental cues', () => {
    const cues = [cue(1, 1), cue(-1, 9), cue(0, Infinity), cue(NaN, 9), cue(1, 3, '')];
    expect(activeCue(cues, 2)).toEqual({ cue: cues[4], index: 4, progress: 0.5 });
    for (const time of [-1, NaN, Infinity, -Infinity]) expect(activeCue(cues, time)).toEqual({ cue: null, index: -1, progress: 0 });
...
describe('beat quantization and playback rate', () => {
  ...
  it('returns unity for invalid inputs and bounded rates for extreme finite inputs', () => {
    ...
    expect(calculatePlaybackRate(Number.MAX_VALUE, Number.MAX_VALUE, Number.MIN_VALUE)).toBeCloseTo(4);
    expect(calculatePlaybackRate(Number.MIN_VALUE, Number.MIN_VALUE, Number.MAX_VALUE)).toBeCloseTo(0.25);
  });
});

describe('asset similarity', () => {
  it('uses normalized tag overlap, energy and wrapping hue', () => {
    const source = asset('source', { tags: [' Neon ', 'FAST'], hue: 359, energy: 0.9 });
    const same = asset('same', { tags: ['neon', 'fast', 'fast'], hue: 1, energy: 0.9 });
    const opposite = asset('opposite', { tags: ['NEON', 'fast'], hue: 179, energy: 0.9 });
    const unrelated = asset('unrelated', { hue: 359, energy: 0.9 });
    expect(similarAssets(source, [unrelated, opposite, source, same]).map(item => item.id)).toEqual(['same', 'opposite', 'unrelated']);
    expect(similarAssets(asset('s'), [asset('b', { energy: 0 }), asset('a')])[0].id).toBe('a');
  });

  it('normalizes negative/multi-turn hue and breaks ties deterministically by ID', () => {
    const source = asset('source', { hue: -1 });
    const candidates = [asset('z', { hue: 359 }), asset('a', { hue: 719 }), asset('opposite', { hue: 179 })];
    expect(similarAssets(source, candidates).map(item => item.id)).toEqual(['a', 'z', 'opposite']);
    expect(similarAssets(source, [...candidates].reverse())).toEqual(similarAssets(source, candidates));
  });

  it('excludes self/duplicate IDs, limits results, handles bad numbers, and never mutates', () => {
    const source = asset('source');
    const first = asset('a');
    const candidates = [source, first, first, asset('bad', { hue: NaN, energy: Infinity }), ...Array.from({ length: 10 }, (_, i) => asset(`b${i}`))];
    ...
    for (const limit of [0, -1, NaN, Infinity]) expect(similarAssets(source, candidates, limit)).toEqual([]);
...
describe('tap tempo', () => {
  ...
  it('ignores duplicate/rapid/backward/nonfinite taps and restarts after inactivity', () => {
    const tap = new TapTempo();
    tap.tap(1000);
    for (const time of [1000, 1100, 900, -1, NaN, Infinity]) expect(tap.tap(time)).toBeNull();
...
describe('timer-free transport', () => {
  ...
  it('depends only on absolute clock anchors, not frame count or frame spacing', () => {
    ...
    for (let frame = 1; frame <= 10000; frame++) dense.getTime(100 + frame * 16.6667);
