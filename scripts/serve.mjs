...
const { createLocalServer } = createRequire(import.meta.url)(
  "../desktop/server.cjs",
);
...
if (
  ...
  console.error(
    "Invalid arguments. Use --port 0..65535. The host is always 127.0.0.1.",
  );
  process.exitCode = 1;
} else {
  let local;
  try {
    ...
    console.error(
      error.code === "ENOENT"
        ...
        : `Could not start Lumina Live: ${error.message}`,
    );
