  ...
  Object.freeze({
    ...
    onRemote: (callback) => {
      if (typeof callback !== "function")
        throw new TypeError("Remote callback must be a function");
      const listener = (_event, message) => {
        if (
          ...
          !message.args.every(Number.isFinite)
        )
          return;
...
function externalClick(event) {
  if (!event.isTrusted || (event.type === "auxclick" && event.button !== 1))
    return;
  const anchor = event
    .composedPath()
