...
const SECURITY_HEADERS = {
  ...
  "Content-Security-Policy":
    "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; media-src 'self' blob:; connect-src 'self'; font-src 'self' data:; worker-src 'self' blob:; object-src 'none'; frame-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'",
};
...
function decodePath(target) {
  ...
  try {
    ...
    throw new HttpError(400, "Invalid encoding");
  }
...
async function inspectFile(root, parts) {
  ...
  if (rootInfo.isSymbolicLink() || !rootInfo.isDirectory())
    throw new HttpError(403, "Forbidden file");
...
async function openSafeFile(root, parts, identity) {
  ...
  try {
    ...
    if (
      !stat.isFile() ||
      stat.dev !== before.info.dev ||
      stat.ino !== before.info.ino ||
      stat.dev !== after.info.dev ||
      stat.ino !== after.info.ino ||
      before.realFile !== after.realFile ||
      (identity &&
        (identity.dev !== stat.dev ||
          identity.ino !== stat.ino ||
          identity.realFile !== after.realFile))
    ) {
...
function validateShowJson(data) {
  if (
    typeof data !== "string" ||
    data.length > MAX_JSON_BYTES ||
    Buffer.byteLength(data, "utf8") > MAX_JSON_BYTES
  )
    throw new Error("Show must be JSON of at most 10 MiB");
  let show;
  try {
    show = JSON.parse(data);
  } catch {
    throw new Error("Invalid show JSON");
  }
  if (
    !show ||
    typeof show !== "object" ||
    ...
    !Array.isArray(show.assets) ||
    !Array.isArray(show.songs) ||
    !Array.isArray(show.decks)
  )
    throw new Error("Invalid Lumina show");
  return data;
}

async function readShowFile(file) {
  if (!path.isAbsolute(file) || path.extname(file).toLowerCase() !== ".json")
    throw new Error("Select a JSON show");
  const root = path.parse(file).root;
  const opened = await openSafeFile(
    root,
    file.slice(root.length).split(path.sep),
  );
  try {
    return validateShowJson(await readLimited(opened.handle, MAX_JSON_BYTES));
  } finally {
...
function assetRootsFor({
  ...
  return [
    ...new Set(
      [
        portableDir && path.join(portableDir, "assets"),
        executableDir && path.join(executableDir, "assets"),
...
function cleanCatalog(value) {
  ...
  return entries.flatMap((entry) => {
    if (
      ...
      ids.has(entry.id) ||
      !["procedural", "image", "video"].includes(entry.kind)
    )
      return [];
    ...
    const asset = {
      ...
      license:
        typeof entry.license === "string" ? entry.license : "Unspecified",
    };
    for (const key of ["visual", "attribution", "status"])
      if (typeof entry[key] === "string") asset[key] = entry[key];
    for (const key of ["bpm", "beats", "duration", "bytes", "seed"])
      if (Number.isFinite(entry[key])) asset[key] = entry[key];
...
function importedAsset(id, record, origin) {
  return {
    ...
    kind: MEDIA_TYPES[record.extension].startsWith("video/")
      ? "video"
      : "image",
    url: `${origin}/media/${id}`,
    tags: ["imported"],
...
async function saveRegistry(file, records) {
  ...
  const normalize = (value) =>
    process.platform === "win32"
      ...
      : path.resolve(value);
  if (normalize(realParent) !== normalize(parent))
    throw new Error("Registry folder must not be a symbolic link");
  const data = JSON.stringify({
    version: 1,
    media: [...records].map(([id, record]) => ({
      id,
      file: record.file,
      realFile: record.realFile,
      dev: record.dev,
      ino: record.ino,
      bytes: record.bytes,
    })),
  });
  if (Buffer.byteLength(data) > MAX_JSON_BYTES)
    throw new Error("Imported media registry is full");
...
async function loadRegistry(file, records) {
  ...
  let opened;
  try {
    opened = await openSafeFile(root, file.slice(root.length).split(path.sep));
  } catch (error) {
  ...
  try {
    ...
    if (
      ...
      value.media.length > 4096
    )
      throw new Error("Invalid imported media registry");
    for (const entry of value.media) {
      if (
        ...
        typeof entry.realFile !== "string" ||
        !path.isAbsolute(entry.realFile) ||
        ![entry.dev, entry.ino, entry.bytes].every(
      ...
      records.set(entry.id, {
        ...
        extension,
        root: entryRoot,
        parts: entry.file.slice(entryRoot.length).split(path.sep),
      });
    }
  } finally {
    await opened.handle.close();
...
async function createLocalServer(options = {}) {
  ...
  const assetRoots =
    options.assetRoots || assetRootsFor({ ...options, rootDir });
  ...
  for (const root of assetRoots) {
    ...
    try {
      ...
      catalog = cleanCatalog(
        JSON.parse(await readLimited(opened.handle, MAX_CATALOG_BYTES)),
      );
      activeAssetRoot = path.resolve(root);
      break;
    } catch (error) {
      if (error.code !== "ENOENT")
        throw new Error("Cannot read asset catalog", { cause: error });
    } finally {
  ...
  async function respond(req, res) {
    ...
    if (req.headers.referer) {
      try {
        if (new URL(req.headers.referer).origin !== origin) throw new Error();
      } catch {
        throw new HttpError(403, "Invalid referrer");
      }
    }
    ...
    if (route === "/assets/catalog.json") {
      send(req, res, 200, JSON.stringify(catalog), {
    ...
    if (parts[0] === "media") {
      ...
      if (!activeAssetRoot || parts.length < 3)
        throw new HttpError(404, "No media");
    ...
    try {
      ...
      if (
        ...
        (!req.headers["if-none-match"] &&
          ...
          Date.parse(modified) <= Date.parse(req.headers["if-modified-since"]))
      ) {
      ...
      if (
        ...
        try {
          ...
          send(req, res, 416, "Range not satisfiable", {
      ...
      await pipeline(
        handle.createReadStream({ ...(range || {}), autoClose: false }),
        res,
      );
    } finally {
      await handle.close();
      ...
      return structuredClone(catalog)
        ...
        .concat(
          [...registered].map(([id, record]) =>
            importedAsset(id, record, origin),
          ),
        );
    },
      ...
      for (const asset of show.assets) {
        ...
        for (const field of ["url", "thumbnail"]) {
          ...
          try {
            ...
            if (match && registered.has(match[1]))
              asset[field] = `${origin}/media/${match[1]}`;
            else if (
              url.pathname.startsWith("/assets/media/") &&
              catalogUrl(url.pathname)
            )
              asset[field] = origin + catalogUrl(url.pathname);
          } catch {
            /* Leave application-level validation to the show loader. */
          }
        }
      }
    ...
    async registerMedia(file) {
      const registration = registryQueue.then(async () => {
        if (
          ...
          throw new Error("Cannot import media");
        ...
        try {
          ...
          for (const [id, record] of registered) {
            if (
              record.realFile === opened.realFile &&
              record.dev === opened.stat.dev &&
              record.ino === opened.stat.ino
            )
...
function parseOscMessage(packet) {
  ...
  function string() {
    ...
    for (let i = offset; i < end; i++)
      if (packet[i] < 0x20 || packet[i] > 0x7e) throw new Error();
  ...
  try {
    ...
    if (
      !/^\/lumina\/(play|stop|next|prev|tap|blackout|crossfade|master|clip)$/.test(
        address,
      )
    )
      return null;
    ...
    if (
      !/^,[if]?$/.test(types) ||
      offset + (types.length - 1) * 4 !== packet.length
    )
      return null;
    const args =
      types.length === 1
        ...
        : [
            types[1] === "i"
              ? packet.readInt32BE(offset)
              : packet.readFloatBE(offset),
          ];
    if (args.some((value) => !Number.isFinite(value))) return null;
    const action = address.slice(8);
    if (action === "clip") {
      if (args.length !== 1 || types !== ",i" || args[0] < 0 || args[0] > 99999)
        return null;
      ...
      (args[0] < 0 ||
        args[0] > 1 ||
        (action === "blackout" && args[0] !== 0 && args[0] !== 1))
    )
      return null;
    return { address, args };
  } catch {
